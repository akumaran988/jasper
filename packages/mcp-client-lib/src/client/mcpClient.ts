...
export class MCPClient {
  ...
  async discoverTools(): Promise<MCPTool[]> {
    ...
    try {
      ...
      this.discoveredTools = response.tools.map((tool: Tool) => {
  ...
  private async createTransport(): Promise<Transport> {
    ...
    throw new Error(
      `Invalid MCP server configuration: missing httpUrl, url, or command for ${this.serverName}`
    );
  }

  private generateValidToolName(name: string): string {
